...
switch ($action)
{
	default: case "view_current_dean":
			...
			switch ($current_deanship_period["status"])
			{
				...
				case "voting":
					...
					$deanship_html = template(
						...
						array(
							"period_id" => $current_deanship_period["id"],
							"from" => $current_deanship_period["from_period"],
							"to" => $current_deanship_period["to_period"],
							"vote" => $vote_html,
						)
		...
		// Who can be dean.
		$who_can_be_dean_html = "";
		$can_be_dean_count = "";

		/*
		// Get the members ok to be deans.					
		$get_members_can_be_dean_query = mysql_query("SELECT member.* FROM member, user WHERE user.member_id = member.id AND member.is_alive = '1' AND member.gender = '1' AND (member.age >= 45 AND member.age <= 100) AND user.first_login = '0' ORDER BY RAND()");
		$can_be_dean_count = mysql_num_rows($get_members_can_be_dean_query);
				
		if ($can_be_dean_count > 0)
		{
			while ($can_dean = mysql_fetch_array($get_members_can_be_dean_query))
			{
				$who_can_be_dean_html .= "<tr><td><a href='familytree.php?id=$can_dean[id]'>$can_dean[fullname]</a></td></tr>\n";
			}
		}
		*/
		...
		$content = template(
			...
			array(
				...
				"can_be_dean_count" => $can_be_dean_count,
				"who_can_be_dean" => $who_can_be_dean_html,
			)
		);
	...
	case "vote_to_member":
		...
		$period_id = mysql_real_escape_string(@$_POST["period_id"]);
		$member_id = mysql_real_escape_string(@$_POST["member_id"]);
		
		// Check if the period is correct.
		$get_deanship_period_query = mysql_query("SELECT * FROM deanship_period WHERE id = '$period_id'");
		...
		// Get the deanship period.
		$deanship_period = mysql_fetch_array($get_deanship_period_query);
		
		// Check if the dean as member correct.
		$dean_member = get_member_id($member_id);
		
		if ($dean_member == false)
		{
			echo error_message("لا يمكنك الوصول إلى هذه الصفحة.");
			return;
		}
		
		// Get the dean or insert a new one.
		$get_dean_query = mysql_query("SELECT * FROM dean WHERE period_id = '$deanship_period[id]' AND member_id = '$dean_member[id]'");
		
		if (mysql_num_rows($get_dean_query) == 0)
		{
			// Insert a new one.
			$now = time();
			$insert_new_dean_query = mysql_query("INSERT INTO dean (period_id, member_id, created) VALUES ('$deanship_period[id]', '$dean_member[id]', '$now')");
			$dean_id = mysql_insert_id();
		}
		else
		{
			$fetch_dean = mysql_fetch_array($get_dean_query);
			$dean_id = $fetch_dean["id"];
		}
	...
	case "voting_results":
		...
		$get_period_deans_query = mysql_query("SELECT dean.id AS id, dean.member_id AS member_id, dean.selected AS selected, (SELECT COUNT(member_dean.id) FROM member_dean WHERE member_dean.dean_id = dean.id) AS votes FROM dean WHERE period_id = '$fetch_deanship_period[id]' ORDER BY votes DESC LIMIT 1");
		$deans_html = "";
		...
		else
		{
			while ($dean = mysql_fetch_array($get_period_deans_query))
			{
				...
				$deans_html .= "<tr><td><a href='family_dean.php?action=view_dean_info&id=$dean[id]'>$dean_info[fullname]</a> $selected</td><td><span class='number'>$percent%</span></td><tr>";
			}
		}
